...
if __name__ == "__main__":
    ...
    #idea: when you remove a rectantangular prism (RP) from an RP, you leave rectangular prisms behind (from 0 to 26). if you add an RP that overlaps another RP, you are adding 1, 2, 3 or 7 sub RPs
    #Question: how to deal/detect RPs that intersect but where none of the corners are internal to the other RP

        newrects = set()
        if instruction = "on":
            newrects.add((x1, x2, y1, y2, z1, z2))
            for r in rects:
                newrects |= add((x1, x2, y1, y2, z1, z2), r)
        else:
            for r in rects:
                newrects |= remove((x1, x2, y1, y2, z1, z2), r)
