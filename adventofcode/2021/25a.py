...
def evolve(downs, rights, maxx, maxy):
    newdowns, newrights = set(), set()
    changed = False
    for cuke in rights:
        newspace = ((cuke[0] + 1) % maxx, cuke[1])
        if newspace in downs or newspace in rights:
            newrights.add(cuke)
        else:
            newrights.add(newspace)
            changed = True
    for cuke in downs:
        newspace = (cuke[0], (cuke[1] + 1) % maxy)
        if newspace in downs or newspace in newrights:
            newdowns.add(cuke)
        else:
            newdowns.add(newspace)
            changed = True
    return (newdowns, newrights, changed)

if __name__ == "__main__":
    data = load_data("25.data")

    downs, rights = set(), set()
    ...
    steps = 0
    while changed:
        downs, rights, changed = evolve(downs, rights, maxx, maxy)
        steps += 1
