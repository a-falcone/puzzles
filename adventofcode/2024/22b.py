...
if __name__ == "__main__":
    ...
    for number in data:
        ...
        previous = []
        for _ in range(2000):
            nextnumber = evolve(number)
            prevprice = number % 10
            price = nextnumber % 10
            previous.append(price - prevprice)
            if len(previous) >= 4:
                prev4 = tuple(previous[-4:])
                if prev4 not in monkeyprofit:
                    monkeyprofit[prev4] = price
            number = nextnumber
       
        for p in monkeyprofit:
            if p in profit:
                profit[p] += monkeyprofit[p]
            else:
                profit[p] = monkeyprofit[p]
