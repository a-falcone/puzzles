...
def first_level(code):
    NUMPAD = {'7': (0,0), '8': (0,1), '9': (0,2), '4': (1,0), '5': (1,1), '6': (1,2), '1': (2,0), '2': (2,1), '3': (2,2), '0': (3,1), 'A': (3,2)}
    pos = 'A'
    path = ''
    for dest in code:
        posy, posx = NUMPAD[pos][0], NUMPAD[pos][1]
        desty, destx = NUMPAD[dest][0], NUMPAD[dest][1]
        dy = desty - posy
        dx = destx - posx
        ...
        if (desty, posx) not in NUMPAD.values():
            path += xchar * abs(dx) + ychar * abs(dy) + 'A'
        elif (posy, destx) not in NUMPAD.values():
            path += ychar * abs(dy) + xchar * abs(dx) + 'A'
        elif dx < 0:
            path += xchar * abs(dx) + ychar * abs(dy) + 'A'
        else:
            path += ychar * abs(dy) + xchar * abs(dx) + 'A'
...
def solve(code, depth):
    ...
    path = 0
    for dest in code:
        posy, posx = KEYPAD[pos][0], KEYPAD[pos][1]
        desty, destx = KEYPAD[dest][0], KEYPAD[dest][1]
        dy = desty - posy
        dx = destx - posx
        ...
        if (desty, posx) not in KEYPAD.values():
            path += solve(xchar * abs(dx) + ychar * abs(dy) + 'A', depth - 1)
        elif (posy, destx) not in KEYPAD.values():
            path += solve(ychar * abs(dy) + xchar * abs(dx) + 'A', depth - 1)
        elif dx < 0:
            path += solve(xchar * abs(dx) + ychar * abs(dy) + 'A', depth - 1)
        else:
            path += solve(ychar * abs(dy) + xchar * abs(dx) + 'A', depth - 1)
...
if __name__ == "__main__":
    ...
    total = 0
    for code in codes:
        total += int(code[:-1]) * solve(first_level(code), 25)
    print(total)
