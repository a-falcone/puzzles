...
def reverse_engineer(program):
    ...
    for x in range(len(program)):
        ...
        for dA in range(1000):
            s = program[-x-1:]
            if run_computer(A + dA, program) == s:
                A += dA
                break
        else:
            print("Didn't find one", s)
    return A

def run_computer(A, program):
    register['A'] = A
    register['B'] = 0
    register['C'] = 0
...
if __name__ == "__main__":
    register, program = load_data("17.data")

    A = reverse_engineer(program)

    output = run_computer(A, program)
