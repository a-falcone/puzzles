...
for inst in instructions:
    if inst.startswith("deal into"):
        deck.reverse()
    elif inst.startswith("cut"):
        pos = int(re.search("cut (-?\d+)", inst).group(1))
        deck = deck[pos:] + deck[:pos]
    elif inst.startswith("deal with"):
        inc = int(re.search("deal with increment (\d+)", inst).group(1))
        pos = 0
        tmp = deck[:]
        for i in range(len(deck)):
            tmp[pos % len(deck)] = deck[i]
            pos += inc
        deck = tmp
    else:
