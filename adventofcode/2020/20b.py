...
def rotate(f):
    ...
    size = len(f)
    for i in range(size):
        d.append("".join([line[size - i - 1] for line in f]))
    return d
...
for row in range(size):
    for tilerow in range(1, len(tiles[solution[0]]._data) - 1):
        field.append("")
        for column in range(size):
            field[-1] += tiles[solution[row*size + column]]._data[tilerow][1:-1]
...
for i in range(2):
    for j in range(4):
        c = 0
        for y in range(len(field) - len(monster)):
            for x in range(len(field[0]) - len(monster[0])):
                found = True
                for loc in m:
                    if field[y + loc[0]][x + loc[1]] != "#":
                        found = False
                    
                if found:
                    c += 1
        if c:
            print( field_count - c * len(m) )
            exit()
        field = rotate(field)
    field = flip(field)
