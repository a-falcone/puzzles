...
def play( p1, p2 ):
    ...
    while len(p1) * len(p2) > 0:
        if tuple( [tuple( p1 ), tuple( p2 )] ) in seen:
            return( {"winner": 1, "p1": p1, "p2": p2} )
        seen.add( tuple( [tuple( p1 ), tuple( p2 )] ) )
        c1 = p1.pop(0)
        c2 = p2.pop(0)
        if len(p1) >= c1 and len(p2) >= c2:
            outcome = play( p1[0:c1], p2[0:c2] )
            if outcome["winner"] == 1:
                p1.extend([c1,c2])
            else:
                p2.extend([c2,c1])
    ...
    if len(p1):
        ...
        winner = 2
    return( {"winner": winner, "p1": p1, "p2": p2} )
...
for line in data:
    ...
    if line.startswith("Player"):
        player = int(line.strip(":").split(" ")[1])
    else:
        p[player].append(int(line))

outcome = play( p[1], p[2] )
winner = ""
if outcome["winner"] == 1:
    winner = "p1"
else:
...
total = 0
for i, c in enumerate( outcome[winner][::-1] ):
    total += (i + 1) * c
print( total )
