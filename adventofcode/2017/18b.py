...
class Computer():
    ...
    def run(self, queue):
        ...
        while True:
            ...
            if args[0] not in self.register:
                self.register[args[0]] = 0

            if instruction == 'snd':
                self.to_send.append(self.value(args[0]))
            elif instruction == 'set':
                self.register[args[0]] = self.value(args[1])
                ...
                if len(self.queue) == 0:
                    return self.to_send
                self.register[args[0]] = self.queue.pop(0)
            elif instruction == 'jgz':
                if self.value(args[0]) > 0:
                    self.pointer += self.value(args[1])
                    continue
...
def load_data(filename: str) -> list:
    ...
    with open(filename, "r") as f:
        for line in f:
            ...
            args = [int(arg) if (arg.isdigit() or arg[0] == '-') else arg for arg in args]
