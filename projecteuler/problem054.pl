...
sub handvalue {
  ...
  my $flush = $hand =~ /^.(.)(.\1){4}$/;
  $hand =~ s/(.)./$1/g;
  $hand =~ tr(AKQJT98765432)
             (ZYXWVUTSRQPON);
  $hand = join "", reverse sort (split //, $hand);
  my $straight = is_straight( $hand );

  if ( $flush && $straight ) { return "Z".substr( $hand, 0, 1 ); }
  elsif ( $hand =~ /(.)\1{3}/ ) { return "Y$1"; }
  elsif ( $hand =~ /(.)\1\1(.)\2/ ) { return "X$1"; }
  elsif ( $hand =~ /(.)\1(.)\2\2/ ) { return "X$2"; }
  elsif ( $flush ) { return "W".substr( $hand, 0, 1 ) }
  elsif ( $straight ) { return "V".substr( $hand, 0, 1 ) }
  elsif ( $hand =~ /(.)\1{2}/ ) { return "U$1"; }
  elsif ( $hand =~ /(.)\1.*(.)\2/ ) { 
    my $tmp = "T$1$2";
    $hand =~ s/(.)\1//g;
    return "$tmp$hand";
...
while ( <DATA> ) {
  ...
  /^(.{10})(.{10})$/;
  my ( $ahand, $bhand ) = ( $1, $2 );
  my $result = handvalue( $ahand ) cmp handvalue( $bhand );
  if ( $result == 1 ) { $awins++ }
  elsif ( $result == 0 ) { $ties++ }
  else { $bwins++ }
}
